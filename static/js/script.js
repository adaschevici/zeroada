...
function initializeOnLoad() {
  ...
  // 2. Termynal Initialization Logic
  ("use strict");

  /**
   * HTML API: If current script has container(s) specified, initialise Termynal.
   * Assuming 'Termynal' is defined globally or imported elsewhere.
   */
  let lines = Array.from(document.querySelectorAll("[data-ty]"));
  lines.forEach((line) => {
    let container = line.closest("[data-termynal]");
    // Check if the Termynal class exists before attempting to instantiate
    if (container && typeof Termynal !== "undefined") {
      new Termynal(container);
    }
  });
}

// Check the document state and attach the combined function appropriately.
if (document.readyState === "loading") {
  // If the DOM hasn't loaded yet, wait for the event.
  document.addEventListener("DOMContentLoaded", initializeOnLoad);
} else {
  // If the DOM has already loaded (or is interactive/complete), run immediately.
  // This covers cases where the script loads late.
  initializeOnLoad();
}
